 ...
 * There may be more than one path with the same diameter which may not include root.
...
public class BinaryTreeDiameter<Item extends Comparable<Item>> {
	...
	private BinaryTreeDiameter<Item>.Node insert(BinaryTreeDiameter<Item>.Node root2,
			Item key) {
		if (root2 == null)
			return new Node(key);
